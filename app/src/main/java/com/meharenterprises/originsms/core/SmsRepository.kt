...
class SmsRepository(private val context: Context) {
    ...
    suspend fun getThreadIdForAddress(address: String): Long = withContext(Dispatchers.IO) {
        val cursor = context.contentResolver.query(android.provider.Telephony.Sms.CONTENT_URI, arrayOf(android.provider.Telephony.Sms.THREAD_ID), "${android.provider.Telephony.Sms.ADDRESS} = ?", arrayOf(address), "${android.provider.Telephony.Sms.DATE} DESC LIMIT 1")
